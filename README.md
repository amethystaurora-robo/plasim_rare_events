...
## Methods
The PlaSim-LSG simulations continue the conclusions of [4], with the applica-
tion of a rare event algorithm to induce N-tipping. As such, parameters selected
for PlaSim-LSG follow those described in [4], where the resampling time of the
rare event algorithm is Ta = 1 year and biasing parameter is k=−3, with
...
first 1000 years were discarded as a transient period. Snapshots of the full sys-
tem state were selected at evenly spaced intervals in the control run, and used
...
surf= rss + rls + hf ls + hf ss 
where rss is the total solar radiation flux, rls is the total thermal radiation
flux, hf ls is the latent heat flux and hf ss is the sensible heat flux. Outward
...
with the timeseries shown in Figure 17.
...
flux and upward thermal radiation. The solar flux shown in Figure 18a shows
...
thermal radiation at the top of the atmosphere shown in Figure 18b decreases
...
budget shown in Figure 18c does not show a noticeable change in the transition
period, meaning that radiative equilibrium is maintained. In the early portion
of the timeseries in Figures 18a and 18c, there are noticeable spikes in radiative

flux, which can also be seen in the energy budget. These may also be worth
...
sphere is shown in Figure 19, where the total change has been calculated as
...
flux, and sensible and latent heat exchanges. Figure 20b represents a surprising
...
emission of the atmosphere is decreasing. The latent heat flux in Figure 19c
shows a decrease in evaporation, whereas the sensible heat flux in Figure 20d
...
in Figure 21, where there is a noticeable increase in radiation off the west coast
of Greenland, and a slight increase in incoming solar radiation around the Gulf
of Mexico, shown in Figure 21a.
<p>
